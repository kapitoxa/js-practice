...
function form() {
  ...
  mainForm.addEventListener('submit', function (event) {
    ...
    var send = sendFormHandler.bind(this);
    send();
  });
...
function modal() {
  ...
  moreBtn.addEventListener('click', function () {
    var show = showOverlay.bind(this);
    show();
  });
  ...
  info.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains('description-btn')) {
      var show = showOverlay.bind(target);
      show();
    }
  });
}
...
function tabs() {
  ...
  function hideTabContent(fromTab) {
    for (var i = fromTab; i < tabContent.length; i++) {
...
function timer() {
  ...
  function getTimeRemainig(deadline) {
    var endtime = Date.parse(deadline);
  ...
  function setClock(elementId, deadline) {
    var timer = document.getElementById(elementId);
    ...
    function updateClock() {
      var time = getTimeRemainig(deadline);
